...
/* BK4BTSTACK_CHANGE STOP */
    ...
    //pstrEncParams->u8NumPacketToEncode = 3; /* default is one for retrocompatibility purpose */
...
void SBC_Encoder_Init(SBC_ENC_PARAMS *pstrEncParams)
{
        ...
        ||  (pstrEncParams->s16ChannelMode == SBC_STEREO) )
    {
        ...
        s16Bitpool = (SINT16)( (pstrEncParams->u16BitRate *
            pstrEncParams->s16NumOfSubBands * 1000 / s16SamplingFreq) //50
            -( (32 + (4 * pstrEncParams->s16NumOfSubBands *
            pstrEncParams->s16NumOfChannels)
            + ( (pstrEncParams->s16ChannelMode - 2) *
            pstrEncParams->s16NumOfSubBands )   )
            / pstrEncParams->s16NumOfBlocks) ); //42
        ...
        s16FrameLen = 4 + (4*pstrEncParams->s16NumOfSubBands*
            ...
            + ( ((pstrEncParams->s16ChannelMode - 2) *
            pstrEncParams->s16NumOfSubBands)
            + (pstrEncParams->s16NumOfBlocks * s16Bitpool) ) / 8; //63
